...
export const api = {
  ...
  confirmVerification: (email: string, code: string) =>
    request<void>("/auth/email-verifications/confirm", {
      method: "POST",
  ...
  remindUsername: (email: string) =>
    request<void>("/auth/username-reminders", {
      method: "POST",
      body: JSON.stringify({ email }),
    }),
  requestPasswordReset: (email: string) =>
    request<void>("/auth/password-resets", {
      method: "POST",
      body: JSON.stringify({ email }),
    }),
  ...
  withdraw: () => request<void>("/auth/me", { method: "DELETE" }, true),

  socialUrl: (provider: "google" | "kakao") =>
    `${API_BASE.replace(/\/api\/v1$/, "")}/oauth2/authorization/${provider}`,
};

export function errorMessage(error: unknown) {
  const value = error as ApiError;
  return (
    value?.message ?? "요청을 처리하지 못했습니다. 잠시 후 다시 시도해 주세요."
  );
}
