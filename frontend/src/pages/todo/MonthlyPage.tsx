...
const loadMonthlyTodos = useCallback(async () => {
  ...
  try {
    ...
    window.alert('월간 일정을 불러오지 못했습니다.');
  }
}, [viewDate]);
...
async function removeTodo(id: number) {
  ...
  return <section className="planner-page monthly-planner">
    ...
    <form className="planner-add-bar" onSubmit={addTodo}><span className="selected-date-label">{selectedDate.getMonth() + 1}월 {selectedDate.getDate()}일</span><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="선택한 날짜에 새 일정을 추가하세요" /><button type="submit">+ 일정 추가</button></form>
