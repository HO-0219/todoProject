...
function RequestPage({ mode }: { mode: "username" | "password" }) {
  ...
  return (
    <AuthLayout
      title={isUsername ? "아이디 찾기" : "비밀번호 찾기"}
      description="가입할 때 사용한 이메일을 입력해 주세요."
    >
      {done ? (
        <section className="success">
          <strong>메일함을 확인해 주세요.</strong>
          <p>가입된 계정이 있다면 안내 메일을 전송했습니다.</p>
        ...
        <form className="form" onSubmit={submit}>
          ...
          <SubmitButton pending={pending}>
            {isUsername ? "아이디 안내 받기" : "재설정 링크 받기"}
          </SubmitButton>
        </form>
      )}
    </AuthLayout>
...
export function FindUsernamePage() {
  return <RequestPage mode="username" />;
}
export function ForgotPasswordPage() {
  return <RequestPage mode="password" />;
}
  ...
  return (
    ...
    >
      <form className="form" onSubmit={submit}>
        ...
        />
        {(!email || !token) && (
          <p className="error">유효한 재설정 링크가 아닙니다.</p>
        )}
        {error && <p className="error">{error}</p>}
        <SubmitButton pending={pending} disabled={!email || !token}>
          비밀번호 변경
        </SubmitButton>
      </form>
    </AuthLayout>
